...
(function (factory) {
	...
	$.fn.jR3DCarousel = function(options){
		var _defaults = {
				...
				perspective: 1,					/* perspective - default is dynamic, perpendicular */
		...
		(function setup(){
			...
			function _createControls(){
				...
				_previousButton.add(_nextButton)
							   .css({position: 'absolute', top:'50%', zIndex:1, transform: 'translateY(-50%)', fontSize: '2em', color: 'rgba(255, 255, 255, 0.97)', cursor:'pointer', userSelect: 'none'})
							   .appendTo(_container)
							   .hide();
		...
		function _fade(targetSlideIndex){
			...
			_rotationAngle = _baseAngle * targetSlideIndex;			
			_jR3DCarouselDiv.css({ transform: 'translateZ('+-_translateZ+'px) rotateY('+-_rotationAngle+'deg)'});
			_jR3DCarouselDiv.css({ opacity: 2 });
		}
